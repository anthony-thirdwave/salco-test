...
<cfif ListLen(CategoryThreadList) GTE "3">
	<cfquery name="GetBC" datasource="#APPLICATION.DSN#">
		SELECT CategoryID,CategoryURL,CategoryAlias,CategoryNAme FROM qry_GetCategoryFrontEndPermissionExtended
		WHERE CategoryID IN (<cfqueryparam value="#ListRest(ListRest(CategoryThreadList))#" cfsqltype="cf_sql_integer" list="yes") AND
