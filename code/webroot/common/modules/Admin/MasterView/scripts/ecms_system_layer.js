...
function ECMS_PF_write_menu(node, left) {
	...
	if (node.child) {
		// collapsed layer
		text += '<layer id="ECMS_' + node.tree + '|' + node.id + '_C" left="' + left + '" visibility="show" z-index="-1">';
		text += ECMS_include_node(node, node.collapsedc, ECMS_include_control_image(node, node.collapsedi));
		text += '</layer>\n';
		// expanded layer
		text += '<layer id="ECMS_' + node.tree + '|' + node.id + '_E" left="' + left + '" visibility="show"  z-index="-1">';
		...
		text += '<layer id="ECMS_' + node.tree + '|' + node.id + '" left="' + left + '" visibility="show"  z-index="-1">';
