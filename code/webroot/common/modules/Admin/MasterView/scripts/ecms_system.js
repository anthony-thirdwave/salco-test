...
if (document.getElementById) {
	...
	document.writeln('<script language="Javascript" type="text\/javascript" src="/common/scripts/outline/ecms_system_id.js"><\/script>');
} else if (document.layers) {
	// layers (Netscape 4, 3)
	document.writeln('<script language="Javascript" type="text\/javascript" src="/common/scripts/outline/ecms_system_layer.js"><\/script>');
} else {
	// default - uses UL/LI display
	document.writeln('<script language="Javascript" type="text\/javascript" src="/common/scripts/outline/ecms_system_default.js"><\/script>');
}
...
// CSS classes
var ECMS_class_bullet = 'MVNav';
var ECMS_class_collapsed = 'MVNav';
var ECMS_class_expanded = 'MVNav';
...
// remove <b>, <B>, </b>, </B> from strings  --PWK, 10/31/01
function remove_b_tags(targetString) {
	return targetString.replace(/\<\/?[Bb]\>/g, "");
}


// new menu_tree object
function ECMS_menu_node(id, name, url, target, expanded, title, cssclass) {
	this.id			= id.toString();
...
function ECMS_add_item(level, name, url, target, expanded, title, cssclass, node) {
	if (!node) {
		if (!ECMS_menu_tree[ECMS_menu_tree_index]) {
			// first item
			ECMS_menu_tree[ECMS_menu_tree_index] = new ECMS_menu_node(0, name, url, target, expanded, title, cssclass);
			// menu settings
	...
	if (level == 0) {
		...
		node.next = new ECMS_menu_node(id, name, url, target, expanded, title, cssclass);
	} else if (node.child) {
		// recurse on next level
		ECMS_add_item(level - 1, name, url, target, expanded, title,cssclass, node.child);
	} else {
		// new child
		node.child = new ECMS_menu_node(node.id + '-0', name, url, target, expanded, title, cssclass);
	}
}
...
function ECMS_include_image(node, i) {
	...
	var theimage;
	if (i==0) {
		theimage='/common/images/admin/icon_minus.gif';
	}
	else if (i==1) {
		theimage='/common/images/admin/icon_plus.gif';
	}
	else {
		theimage='/common/images/admin/icon_document.gif';
	}
	
	//theimage='/common/images/spacer_9x13.gif';
	if (image) {
		var alt = (node.child ? node.name + ' ' + image.alt : image.alt);

		// need to regex out <b> tags from the var alt  --PWK, 10/31/01
		var text_alt = remove_b_tags(alt);

		//alert(image.src);
		//return '<img src="/common/images/wgt_submenu_arrow_right.gif" ' + 'border="0" alt="' + alt + '" ID="ECMS_' + node.tree + '|' + node.id + '_IMG">';
		return '<img src="' + theimage + '" border="0" alt="' + text_alt + '" ID="ECMS_' + node.tree + '|' + node.id + '_IMG" />';
...
function ECMS_include_control_image(node, image) {
	//alert(ECMS_include_image(node, image));
...
function ECMS_include_control_link(node, text) {
	thistitle=node.title;

	// need to regex out <b> tags from the title  --PWK, 10/31/01
	var text_title = remove_b_tags(thistitle);

	return '<a href="javascript:ECMS_control_click(' + node.tree + ', \'' + node.id + '\');" title="' + text_title + '" target="_self"' + ECMS_include_behaviours(node) + '>' + text + '</a>';
}
...
function ECMS_include_node_link(node, display) {
	...
	if (node.url) {
		...
		text = '<a href="' + node.url + '" title="' + text_title + '"' + (node.target ? ' target="' + node.target + '"' : '') + (node.cssclass ? ' class="' + node.cssclass + '"' : '') + ECMS_include_behaviours(node) + '>' + text + '</a>';
