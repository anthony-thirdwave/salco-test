...
<cfparam name="url.userdir" default="/common/incoming/">
<cfif url.userdir EQ "/common/incoming/">
	<!--- Put file right into the incoming folder. TPS --->
	<!--- <Cfset thisSubDir =  url.type> --->
<cfelse>
	<cfif URL.Type IS "image">
		<cfset thisSubDir = "images">
	<cfelseif URL.Type IS "file">
		<cfset thisSubDir = "documents">
	</cfif>
</cfif>
...
<cfset xmlContent = "<Error number=""1"" text=""#userFilesServerPath#"" />">
...
<cftry>
	...
	<!--- create sub-directory for file type if it doesn't already exist --->
		<cfif not directoryExists(userFilesServerPath & thisSubDir)>	
		<cfdirectory action="create" directory="#userFilesServerPath##thisSubDir#" mode="755">
	</cfif>
...
<cfif not len(xmlContent)>
	...
	<cfset currentFolderPath = userFilesServerPath & thisSubDir & replace(url.currentFolder,"/",fs,"all")>
	<cfset currentFolderPath = Replace(currentFolderPath,"\\","\","all")>
...
<cfset XMLURL = "#userFilesPath##thisSubDir##url.currentFolder#">
<cfset XMLURL = Replace(XMLURL,"//","/","all")>
<cfscript>
	xmlHeader = '<?xml version="1.0" encoding="utf-8" ?><Connector command="#url.command#" resourceType="#url.type#">';
	xmlHeader = xmlHeader & '<CurrentFolder path="#url.currentFolder#" url="#XMLURL#" />';
	xmlFooter = '</Connector>';
</cfscript>
