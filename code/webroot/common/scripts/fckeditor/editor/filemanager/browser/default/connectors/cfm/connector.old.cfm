<!--- The next 2 lines are to not let just anyone brose the server -Dan Kubica --->
...
<cfset sUserFilesURL="#Replace(userdir, '../', '', 'ALL')#" />
<cfset sUserFilesURL = "#Replace(sUserFilesURL, '..\', '', 'ALL')#" />
<cfif sUserFilesURL EQ "">
	<cfset sUserFilesURL = "/">
</cfif>
...
<cfscript>
	...
	sXMLHeader = sXMLHeader & '<CurrentFolder path="#URL.CurrentFolder#" url="#sUserFilesURL##URL.CurrentFolder#" />';
...
<cfif 0>
	<cfif not directoryExists( sUserFilesPath & URL.Type & URL.CurrentFolder )>
		<cfdirectory 
			action="create" 
			directory="#sUserFilesPath##URL.CurrentFolder#"
		>
	</cfif>
</cfif>
...
<cfcase value="FileUpload">
	...
	<cftry>
		...
		<cffile action="UPLOAD"
			fileField="NewFile"
			destination="#sUserFilesPath##URL.CurrentFolder#"
		...
		<cfif compare( CFFILE.ServerFileName, sFileName )>
			...
			<cffile
				action="RENAME"
				source="#sUserFilesPath##URL.CurrentFolder##CFFILE.ServerFileName#.#CFFILE.ServerFileExt#"
				destination="#sUserFilesPath##URL.CurrentFolder##sFileName#.#sFileExt#"
				mode="644"
				attributes="normal"
			>
		</cfif>
