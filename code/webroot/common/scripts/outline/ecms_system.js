...
// CSS classes
var ECMS_class_bullet = 'navsubmenu';
var ECMS_class_collapsed = 'navsubmenu';
var ECMS_class_expanded = 'navsubmenu';
...
function ECMS_include_image(node, i) {
	...
	var theimage;
	if (i==0) {
		theimage='/common/images/ECMS_expanded.gif';
	}
	else if (i==1) {
		theimage='/common/images/ECMS_collapsed.gif';
	}
	else {
		theimage='/common/images/ECMS_bullet.gif';
	}
...
function ECMS_include_control_link(node, text) {
	...
	return '<a class="navsubmenu" href="javascript:ECMS_control_click(' + node.tree + ', \'' + node.id + '\');" title="' + text_title + '" target="_self"' + ECMS_include_behaviours(node) + '>' + text + '</a>';
}
...
function ECMS_include_node_link(node, display) {
	...
	if (node.url) {
		...
		text = '<a class="navsubmenu" href="' + node.url + '" title="' + text_title + '"' + (node.target ? ' target="' + node.target + '"' : '') + ECMS_include_behaviours(node) + '>' + text + '</a>';
...
function ECMS_include_node(node, cl, i) {
	var	text = '<table border="0" cellpadding="1" cellspacing="0"><tr>';
