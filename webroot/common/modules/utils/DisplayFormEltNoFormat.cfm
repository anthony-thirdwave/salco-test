...
<cfif message IS NOT "">
	...
	<cfset Message="<span class=""error"">#Message#</span>">
<cfelse>
	<cfif ATTRIBUTES.FormEltOnly IS "N">
...
<cfelse>
	<cfset BeginningTag="">
	<cfset EndTag="">
	#Attributes.Caption#<cfif attributes.required eq "Y">#attributes.requiredmark#</cfif><cfif Attributes.captionontop is "Y"><br><cfelse> </cfif>
</cfif>
<cfswitch expression="#Trim(ATTRIBUTES.type)#">
	...
	<cfcase value="text,password" delimiters=",">
		...
		<cfif ATTRIBUTES.ObjectAction IS "ShowForm" OR ATTRIBUTES.ObjectAction IS "Validate">
			...
			<cfset ThisValue="#ATTRIBUTES.DefaultValue#">
			<cfif ATTRIBUTES.Date IS "Y" and IsDate(ATTRIBUTES.DefaultValue)>
				<cfset ThisValue=DateFormat(ATTRIBUTES.DefaultValue,"mm/dd/yyyy")>
			</cfif>
			<cfif ShowForm>
				<input type="#Trim(ATTRIBUTES.type)#" name="#ATTRIBUTES.VarName#" value="#htmleditformat(ThisValue)#" size="#ATTRIBUTES.Size#" maxlength="#ATTRIBUTES.MaxLength#" <cfif ATTRIBUTES.FormEltStyle IS NOT "">style="#ATTRIBUTES.FormEltStyle#"</cfif><cfif ATTRIBUTES.FormEltJavaScript IS NOT "">#ATTRIBUTES.FormEltJavaScript#</cfif>>&nbsp;<cfif Len(trim(Message)) IS NOT "0"><BR>#Message#</cfif>
			<cfelse>
				<cfif ATTRIBUTES.CreditCard IS "Y">
		...
		<cfif ATTRIBUTES.ObjectAction IS "ShowForm" OR ATTRIBUTES.ObjectAction IS "Validate">
			...
			<cfelse>
				...
				<cfelseif isdefined("ATTRIBUTES.Tempdir") and ATTRIBUTES.Tempdir IS NOT "">
					...
					<cfif ListFindNoCase(ATTRIBUTES.DocumentExtensionList,".#ListLast(FileServerPath,'.')#") GT "0" and ATTRIBUTES.SupressView IS NOT "Y">
