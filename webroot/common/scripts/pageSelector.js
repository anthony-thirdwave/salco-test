var count = 0, listCount = -1, results, theItem, theHiddenItem, theDiv = "suggest", listID = "articleList", txtBox = "txtResource", oktosubmit=1, thisBGColor = "#FFFFCC";
			...
			while(gogo) {
				if(results[count].indexOf("%%%") == -1){
					if(results[count].indexOf("~|~") != -1){
						...
						eName = "results" + count;
						document.getElementById(theDiv).innerHTML = document.getElementById(theDiv).innerHTML + "<div id=\"" + eName + "bg\" style=\" padding-left: 0px; \"><a class=\"suggest\" id=\"" + eName + "\" style=\"color:#000000; /*margin-left: 5px;*/   \" href=\"javascript:addArticle('" + RID[count] + "','" + escape(JSArticleName) + "');\">" + articleName + "</a></div>";
					}
					count++;
				}else{
					gogo = false;
...
function getSuggestions(letters, e, filter) {
	//alert(e.keyCode);
...
function addArticle(id,name){
	if(elementExists(id)){
		...
		newListElem.innerHTML = "<span class=\"handle\">" + name + "</span> <a href=\"javascript:removeArticle('" + newListElem.getAttribute('id') + "');\">remove</a>";
