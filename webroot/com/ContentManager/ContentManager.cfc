<cfcomponent output="false">
	...
	<cffunction name="GetColumnContentIDList" output="1" returntype="string">
		...
		<cfquery name="LOCAL.GetInherited" datasource="#APPLICATION.DSN#">
			SELECT		ContentID 
		...
		<cfif LOCAL.GetInherited.RecordCount IS "0">
			<cfquery name="LOCAL.GetInherited" datasource="#APPLICATION.DSN#" maxrows="1">
				SELECT		ContentID 
				FROM		qry_GetContentInherit
				WHERE		ContentPositionID=<cfqueryparam value="#Val(ARGUMENTS.ContentPositionID)#" cfsqltype="CF_SQL_INTEGER">
		...
		<cfreturn ValueList(LOCAL.GetInherited.ContentID)>
	</cffunction>
</cfcomponent>